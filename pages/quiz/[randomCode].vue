    ...
    <div class="flex flex-col overflow-hidden grow">
        ...
        <div class="flex flex-col items-center h-screen mt-24 grow">  
            <div 
              ...
              :style="{
                background: settings?.pre_gacha?.quiz?.background_page.type === 'image' ? `url(${settings?.pre_gacha?.quiz?.background_page.value})` : settings?.pre_gacha?.quiz?.background_page.value,
                'background-size': 'cover',
                'background-repeat': 'no-repeat'
              }"  
            >
              <div class="flex flex-col items-center justify-center gap-1">
                <p class="bg-[#341f15] text-white text-[2.5vw] xs:text-[12px] sm:text-[16px] px-3 py-1 rounded-full font-semibold">{{ settings?.pre_gacha?.quiz?.page_sub_title }}</p>
              ...
              <div class="flex flex-col items-center justify-center w-full gap-1">
                <p class="text-[2.5vw] xs:text-[12px] sm:text-[16px] text-[#341f15] text-center px-3 py-2 rounded-full font-bold ">{{ settings?.pre_gacha?.quiz?.answer_box_text }}</p>
                <InputTextArea name="answer" placeholder="Answer" class="w-[68%] h-[25vw] sm:h-28 border border-[#341f15] rounded-md" />
              </div>
              
              <div class="flex items-center w-full gap-0 px-3 py-2 text-white bg-exd-red-vermilion">
                <a :href="settings?.pre_gacha?.quiz?.url_link" class="text-[2.5vw] xs:text-[12px] sm:text-[16px] flex text-center items-center rounded-full font-bold underline cursor-pointer">{{ settings?.pre_gacha?.quiz?.url_link_text }}</a><IconsExport :style="{ color: '#ffffff' }" class="w-6 h-6 ml-[2px] cursor-pointer" />
              </div>
            </div>
            ...
            <div class="fixed bottom-0 z-50 w-full max-w-md px-8 mx-auto mb-1">
              <SolidButton
                ...
                :bgColor="settings?.pre_gacha?.quiz?.button_and_text_color?.background"
